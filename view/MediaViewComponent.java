...
public class MediaViewComponent {
    ...
    public MediaViewComponent(MainPresenter presenter) {
        this.mediaView = new MediaView(presenter.observePlayerState()
                .map(playerState -> playerState.status() != null ? playerState.status().ordinal() == 2 : false)
                .filter(playing -> playing)
                .map(playing -> presenter.stop().blockingGet())
                .map(playerState -> presenter.getMediaPlayer())
                .blockingFirst());
